...
\begin{definition}{Equilibrium Measure}{equilibrium-measure}
  For a given pairwise interaction potential $K: \R \mapsto \R$, the equilibrium measure $\rho: D \mapsto \R$ with $D \subseteq \R^d$ is a measure chosen such that
  $$U_K[\rho] = \frac{1}{2} \iint K\left(\norm{\vec{x} - \vec{y}}\right) \,\dd\rho(\vec{x})\,\dd\rho(\vec{y})\,,$$
...
With these preliminaries in mind, we can define \href{def:the-problem}{the problem}.

% \input{chapters/bits/function-space.tex}  % TODO!

\begin{definition}[colframe=gray, colbacktitle=gray!30]{The Problem}{the-problem}
  Finding an equilibrium measure $\rho: B_R(\vec{0}) \mapsto \R$ of mass $M = 1$ on a $d$-dimensional ball of radius $R \in \R^+$ that minimises the total potential $U_K[\rho]$ given an interaction kernel $K: \R^+ \mapsto \R$.
\end{definition}
% \hierKoennteIhreWerbungStehen
