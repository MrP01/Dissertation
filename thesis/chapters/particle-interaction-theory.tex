...
As throughout the rest of this document, we restrict ourselves to radially symmetric solutions, we will use $\hatvec{x} := R\vec{x} \in B_R(\vec{0})$ to denote position in the original $d$-dimensional ball domain of radius $R$, whereas $\vec{x} \in B_1(\vec{0})$ denotes position in the normalised domain with radius $1$.
...
With these preliminaries in mind, we can now formulate \hyperref[def:the-problem]{the problem}.
...
For an example set of parameters, the resulting simulation output can be found in \Cref{fig:mixed-2d-quiver}.
