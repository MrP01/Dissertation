...
As mentioned in \Cref{chap:particle-interaction-theory}, the numerical approaches will be carried out on the normalised domain $B_1(\vec{0})$ and we will be looking for $\rho \in \functionspace$, whilst in general we are interested in the $\hat{\rho} \in \functionspacehat$ solving \hyperref[def:the-problem]{our problem}.
...
\begin{equation}
  ...
  \label{eq:ansatz}
\end{equation}
with $P_k^{(a, b)}$ the Jacobi polynomials and $\{\rho_k\}_{k=0, ..., N-1}$ the coefficients and $m \in \N_0$ the smallest non-negative integer such that $-d < \alpha < 2 +  2m - d$ in order to ensure that $a = m - \frac{\alpha+d}{2} > -1$ due to \Cref{thm:jacobi-orthogonality-condition}.
...
% \Cref{thm:theorem216} gives an explicit expression for the main integral \(\mathcal{Q}^{\beta}: L \to L\), an operator from the function space \(L\) to the function space \(L\), we are interested in:
...
The normalisation constant is given in \Cref{lemma:mass}, based only on a single coefficient $\rho_0$, allowing for a highly efficient re-normalisation.
