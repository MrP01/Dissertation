...
\begin{equation}
  ...
  \label{eq:ansatz}
\end{equation}
with $P_k^{(a, b)}$ the Jacobi polynomials and $\{\rho_k\}_{k=0, ..., N-1}$ the coefficients and $m \in \N_0$ the smallest nonnegative integer such that $-d < \alpha < 2 +  2m - d$ in order to ensure that $a = m - \frac{\alpha+d}{2} > -1$ due to \Cref{thm:jacobi-orthogonality-condition}.
